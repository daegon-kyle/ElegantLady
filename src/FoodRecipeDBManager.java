...
public class FoodRecipeDBManager 
{
	...
	public void readFoodInfo()
	{
		...
		FoodDAO foodDAO = new FoodDAO();//DAO객체 생성.
		foodList = foodDAO.selectAll(); //table모든 데이터 가져온다.
		foodDAO.disconnect();
		Iterator<FoodDTO> iterator = foodList.getFoodList().iterator();
		while(iterator.hasNext())
		{
			FoodInfo foodInfo = new FoodInfo(); //모델 info
			FoodDTO temp = iterator.next(); //임시 DTO객체
			...
			foodInfoList.addFoodInfoList(foodInfo); //foodInfoList에 추가추가
		}
...
/*		Iterator<FoodInfo> iter = foodInfoList.getFoodInfoList().iterator();
	...
	public void readCookingCourse()
	{
		...
		int tmp = 0;
		CookingCourseList cookingCourseList = null;//코드바뀔때생성
	...
	public void readIngredient()
	{
		...
		int tmp = 0;
		RecipeIngreInfoList recipeIngreInfoList = null;//코드바뀔때생성
